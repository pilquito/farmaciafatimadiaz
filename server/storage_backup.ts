...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByResetPasswordToken(token: string): Promise<User | undefined> {
    ...
    if (!user || !user.resetPasswordExpires || new Date(user.resetPasswordExpires) <= now) {
      return undefined;
    }
  ...
  async deleteContactMessage(id: number): Promise<boolean> {
    ...
    return true; // El método delete de drizzle no devuelve el número de filas afectadas
  }
  ...
  // Métodos para gestión de citas médicas
  async getAppointments(): Promise<Appointment[]> {
...
// Cambiamos el almacenamiento en memoria por uno basado en base de datos
// Cambiamos de almacenamiento en memoria a base de datos persistente
